...
Description: Summed weight of all *xth* individuals listed in the subsample_count column
...
# Chang time zone to Pacific time
LifeHistoryHaulCatch2 <- LifeHistoryHaulCatch %>%
  mutate(
    haulback_time.utc = haulback_time,
    haulback_time.pacific = with_tz(haulback_time, tzone = "America/Los_Angeles"),
    equilibrium_time.utc = equilibrium_time,
    equilibrium_time.pacific = with_tz(equilibrium_time, tzone = "America/Los_Angeles")
    ) %>%
...
sum.tab <- LifeHistoryHaulCatch2 %>% 
  group_by(equilibrium_time.pacific) %>%
...
LifeHistorySpecimen <- LifeHistorySpecimen %>%
  mutate(equilibrium_time.utc = time,
         equilibrium_time.pacific = with_tz(time, tzone = "America/Los_Angeles")
         ) %>%
  select(-time)
...
joinKeys <- LifeHistorySpecimen %>% select(c(1:4,7:8,17:18)) %>% names()
joinKeys
# [1] "cruise"                   "ship"                     "haul"                    
# [4] "collection"               "itis_tsn"                 "scientific_name"         
# [7] "equilibrium_time.utc"     "equilibrium_time.pacific"

# Join data sets by column ids and calculate haul expansion factor
CatchWeightLengthPerHaul <- left_join(LifeHistorySpecimen, LifeHistoryHaulCatch2, 
...
# [16] "mantle_length"            "equilibrium_time.utc"     "equilibrium_time.pacific"
# [19] "start_latitude"           "start_longitude"          "stop_latitude"           
# [22] "stop_longitude"           "equilibrium_time"         "haulback_time"           
# [25] "surface_temp"             "surface_temp_method"      "ship_spd_through_water"  
# [28] "subsample_count"          "subsample_weight"         "remaining_weight"        
...
Haul.weights.oth <- CatchWeightLengthPerHaul %>%
  ...
  # Group by transect
  group_by(collection, ship, haul, equilibrium_time.pacific) %>% 
  mutate(# Set to zero because we only want this as a place holder
         Species_weight = 0, 
...
Haul.weights <- Haul.weights %>%
  group_by(collection, ship, haul, equilibrium_time.pacific, scientific_name) %>%
...
range(Haul.weights$quality.indic)
# [1] NA NA

# See if there are na values
unique(is.na(Haul.weights$quality.indic))
# [1] FALSE  TRUE

# Explore where the NA values occured
unique(is.na(Haul.weights$weight))
# [1] FALSE  TRUE

# Explore where the NA values occured
unique(is.na(Haul.weights$subsample_weight))
# [1] FALSE

# Because there a missing entries in the weight
# column, check if there is an issue with count too
unique(is.na(Haul.weights$subsample_count))
#[1] FALSE
...
wgteq.subs <- Haul.weights %>%
  filter(is.na(weight) & subsample_count == 1) %>%
  mutate(weight = subsample_weight, quality.indic = 0)

Haul.weights2 <- rbind(Haul.weights, wgteq.subs) %>%
 filter(!is.na(weight))
  
unique(is.na(Haul.weights2$quality.indic))
# [1] FALSE  TRUE

test.Haul.weights2 <- Haul.weights2 %>%
...
range(check1$equilibrium_time.pacific)
# [1] "2003-07-10 00:36:00 PDT" "2024-08-03 03:27:00 PDT"
...
Haul.weights2 <- 
  Haul.weights2[,c("scientific_name","quality.indic","FID","subsample_count",
                   "specimen_number","subsample_weight","remaining_weight","weight",
                   "cruise","ship","haul","collection","latitude","longitude",
                   ...
                   "equilibrium_time.pacific","haulback_time.utc","haulback_time.pacific",
                   "surface_temp","surface_temp_method","ship_spd_through_water",
...
range(check2$equilibrium_time.pacific)
# [1] "2003-07-12 01:52:00 PDT" "2024-08-03 03:27:00 PDT"
...
Haul.weights.rev <- Haul.weights2 %>%
...
range(Haul.weights.rev$equilibrium_time.pacific)
# [1] "2003-07-10 00:36:00 PDT" "2024-08-03 03:27:00 PDT"
...
range(test.rev$equilibrium_time.pacific)
# [1] "2003-07-10 00:36:00 PDT" "2019-06-29 22:55:00 PDT"
...
range(Haul.weights.rev.1.1$equilibrium_time.pacific)
# [1] "2003-07-20 04:13:00 PDT" "2003-07-20 04:13:00 PDT"
...
Haul.weights.rev2 <- Haul.weights.rev %>%
  group_by(collection, ship, haul, equilibrium_time.pacific) %>%
  filter(!collection %in% Haul.weights.rev.1.1$collection) %>%
...
#  [1] -0.092  0.000  0.006 -0.003 -0.002 -0.050  0.037 -0.005  4.135  1.395 -0.020
# [12]  0.001  0.072 -0.155  0.003 -0.030  0.100
...
range(precision.error$equilibrium_time.pacific)
# [1] "2003-07-10 00:36:00 PDT" "2019-06-29 22:55:00 PDT"
...
# [25] "stop_longitude"           "equilibrium_time.utc"     "equilibrium_time.pacific"
# [28] "haulback_time.utc"        "haulback_time.pacific"    "surface_temp"            
# [31] "surface_temp_method"      "ship_spd_through_water"   "presence_only"           
# [34] "beginTransect"            "endTransect" 
...
length(unique(Haul.weights2$collection))
# [1] 1896
...
original.collections <- unique(Haul.weights2$collection) 
...
missing.coll <- Haul.weights2 %>% 
  filter(collection %in% index.tab$original.collections)

# Re-estimate quality.indic values
missing.coll <- missing.coll %>%
  group_by(collection, ship, haul, equilibrium_time.pacific) %>%
  mutate(quality.indic = round(sum(weight)-subsample_weight,3))

unique(missing.coll$quality.indic)
# [1] 0

# Join data back since it was missed in the earlier code
Cleaned.data2 <- rbind(Cleaned.data, missing.coll)

length(unique(Cleaned.data2$collection))
# [1] 1896

fwrite(Cleaned.data2, paste(here(dir), "/Data", survey.path,
