...
len.bins3 <- len.bins2 %>%
  ...
  select(cruise, ship, haul, collection, age, len.bin, Catch.wt, log1p.CPUE.trp,
         Year, Month, Season, time.step, geometry, equilibrium_time.pacific, 
...
areaTranBuffsum.prep <- area.transects.buff %>%
  ...
  select(keys, Season, date, tim_stp, clstr_g, geometry) %>%
  distinct() %>%
  ...
  group_by(Season, date, tim_stp, clstr_g, Hauls) %>%
...
area.transects.buff2 <- area.transects.buff %>%
  ...
  distinct() %>%
  # Combine all unique identifier sequences per cluster
  group_by(Season, date, tim_stp, clstr_g, stndrd_, age, len_bin, LON, LAT) %>%
  mutate(Ctch_wt = round(sum(Ctch_wt),2),
         NBRE_IN = round(sum(NBRE_IN)),
         Total_N = round(sum(Total_N)),
         CtchWgL = round(sum(CtchWgL),2)) %>%
  ungroup() %>%
  select(Season, date, tim_stp, clstr_g, len_bin, stndrd_, age, Total_N, NBRE_IN,
...
areaTranBuffsum.new <- inner_join(area.transects.buff2, areaTranBuffsum.prep2, 
                                 by = join_by(Season, date, tim_stp, clstr_g)) %>%
  st_as_sf(., crs = 5070)
...
areaTranBuffsum.new <- areaTranBuffsum.new %>%
  left_join(best_match, by = c("date", "clstr_g"))

sort(unique(areaTranBuffsum.new$Area_id))
# [1] 1 2 3 4 5
...
areaTranBuff.polygons.wgs84 <- areaTranBuffsum.new %>%
  mutate(Area_id = as.factor(Area_id)) %>%
  st_transform(., crs = 4326)
