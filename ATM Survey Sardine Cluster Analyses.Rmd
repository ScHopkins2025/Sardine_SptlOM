...
PreAbs <- rbind(len.bins3, Absent)  %>%
  mutate(date = as.Date(equilibrium_time))
# View(PreAbs)
...
area.transects <- inner_join(PreAbs, sun_times, by = c("date", "LAT", "LON")) %>%
  st_transform(., crs = 5070)
# View(area.transects)  
...
# Where to save output
bufferTrans.dir <- here(getwd(), "Shapefiles")
...
#  [1] "collctn"  "age"      "len_bin"  "Ctch_wt"  "CPUE_tr"  "Year"     "Month"   
#  [8] "Season"   "tim_stp"  "eqlbrm_"  "stndrd_"  "NBRE_IN"  "Total_N"  "LON"     
