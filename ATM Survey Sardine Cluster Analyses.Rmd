...
area.transects.final <- area.transects.final %>%
  select(-c(haulback_time.utc, equilibrium_time.utc, haulback_time.pacific,
...
# Test the buffer was applied correctly; should be false
identical(area.transects.final$geometry, area.transects.buff$geometry)
# [1] FALSE

write_sf(area.transects.buff, 
         file.path(paste(dir, "/Data/Shapefiles/area.transects.w.buffer.shp", 
                         sep = "")),
         overwrite = TRUE)
...
test.area.transects.prestep <- 
  read_sf(file.path(paste(dir, "/Data/Shapefiles/area.transects.w.buffer.shp", 
                          sep = "")
                    ))
...
EntryID <- sort(as.numeric(seq_len(length(unique(Hauls.N3$Year)))))
Year <- sort(as.numeric(substr(unique(Hauls.N3$Year),1,4)))
