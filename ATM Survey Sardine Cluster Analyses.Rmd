...
test <- make_transect_sf(full.series)
...
len.bins3 <- len.bins2 %>%
  ...
  ungroup() %>%
  select(collection, age, len.bin, Catch.wt, log1p.CPUE.trp, Year, Month, Season,
         time.step, geometry, equilibrium_time.pacific, standard_length, NBRE_IN,
         Total_N, LON, LAT, CatchWeightLen) %>%
  mutate(keys = paste(collection, Year, Month, Season, geometry, time.step,
                      equilibrium_time.pacific, sep="")) %>%
  distinct() %>%
  # Because equilibrium_time.pacific is changed to factor, change it back
  # to POSIXct
...
PreAbs$date <- as.character(PreAbs$date)
