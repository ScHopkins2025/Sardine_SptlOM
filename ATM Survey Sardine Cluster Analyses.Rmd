...
len.bins3 <- len.bins2 %>%
  ...
  mutate(
    ...
    NBRE_samp = sum(unique(subsample_count)),
    # haul-level total weight (assumed constant within haul)
    Total_W = sum(unique(Species_weight)),
    .groups = "drop"
  ) %>%
...
# Note that the sum of weight (individual weight obs) should equal the 
# subsample_weight (checked in data prep code)
## Sum the number of sampled individuals and total weight of the catch 
## Note that in this case both are already provided / calculated
len.bins3 <- len.bins2 %>% 
  group_by(cruise, ship, haul, equilibrium_time.pacific, collection, geometry) %>%
  mutate(NBRE_samp = subsample_count, 
         Total_W = Species_weight) %>%
  ungroup() %>%
  # Sum over length bins per haul
  group_by(cruise, ship, haul, equilibrium_time.pacific, collection, geometry, 
           len.bin) %>% 
  mutate(NBRE_bin = n(),
         WGHT_bin = sum(as.numeric(weight))) %>%
  ungroup() %>% 
  # Determine portion of lengths represented in the catch by length bin.
  # Find the mean weight at length.
  group_by(cruise, ship, haul, equilibrium_time.pacific, collection, 
           len.bin, geometry) %>% 
  mutate(PropLen_bin = NBRE_bin/NBRE_samp, 
         TheoreticalWgt_i = (7.52e-06*((standard_length)^3.23))) %>%
  ungroup() %>%
  # Take the proportion of the mean weight at length in the catch 
  group_by(cruise, ship, haul, equilibrium_time.pacific, collection, geometry) %>%
  mutate(EstimatedWeight = sum(PropLen_bin*TheoreticalWgt_i)*NBRE_samp) %>%
  ungroup() %>%
  # Adjust the total weight (sampled + unsampled weight) based on the proportion
  # of mean weight at length
  group_by(cruise, ship, haul, equilibrium_time.pacific, collection, geometry) %>%
  mutate(Correction.factor = Total_W/EstimatedWeight) %>%
  ungroup() %>%
  # Multiple the proportion of lengths represented in the catch by the adjusted
  # proportion of weight represented by length
  group_by(cruise, ship, haul, equilibrium_time.pacific, collection, geometry) %>%
  mutate(CatchWeightLen_bin =
           NBRE_samp*PropLen_bin*TheoreticalWgt_i*Correction.factor) %>%
  ungroup() %>%
  distinct()

View(len.bins3)

range(len.bins3$Total_W)
# [1]0.0005 31819.6497

range(len.bins3$CatchWeightLen_bin)
# [1] 2.378489e-06  1.054911e+03
