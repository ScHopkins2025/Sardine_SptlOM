...
plotNormalHistogram(area2poly.dens$CtchWgL)
range(area2poly.dens$CtchWgL)
...
plot(centroids.t$X, centroids.t$Y) 

# Get summary table for nearest neighbor
library(spatstat)
coords <- cbind(centroids.t$X, centroids.t$Y)
nn_dist <- nndist(coords)
summary(nn_dist)

# Set cut-toff to 2/3 distance (mean or median - choose median if strong out-liers)
...
# 4km
mesh04 <- make_mesh(centroids.t, xy_cols = c("X", "Y"), cutoff = 4)
plot(mesh005)
points(centroids.t$X, centroids.t$Y, col = "red", pch = 19, cex = 0.1)

# 1 km
mesh01 <- make_mesh(centroids.t, xy_cols = c("X", "Y"), cutoff = 1)
plot(mesh01)
points(centroids.t$X, centroids.t$Y, col = "red", pch = 19, cex = 0.1)

# 2 km
mesh02 <- make_mesh(centroids.t, xy_cols = c("X", "Y"), cutoff = 2) 
plot(mesh02)
points(centroids.t$X, centroids.t$Y, col = "red", pch = 19, cex = 0.1)

# 3 km
mesh03 <- make_mesh(centroids.t, xy_cols = c("X", "Y"), cutoff = 3)
plot(mesh05)
points(centroids.t$X, centroids.t$Y, col = "red", pch = 19, cex = 0.1)

# 5 km
mesh05 <- make_mesh(centroids.t, xy_cols = c("X", "Y"), cutoff = 5)  
plot(mesh05)
points(centroids.t$X, centroids.t$Y, col = "red", pch = 19, cex = 0.1)
...
```{r, area2.mesh}
# 15 km
mesh15 <- make_mesh(centroids.t, xy_cols = c("X", "Y"), cutoff = 15000)  
plot(mesh15)
points(centroids.t$X, centroids.t$Y, col = "red", pch = 19, cex = 0.1)

# 20 km
mesh20 <- make_mesh(centroids.t, xy_cols = c("X", "Y"), cutoff = 20000)  
plot(mesh20)
points(centroids.t$X, centroids.t$Y, col = "red", pch = 19, cex = 0.1)

# 25 km
mesh25 <- make_mesh(centroids.t, xy_cols = c("X", "Y"), cutoff = 25000)
plot(mesh25)
points(centroids.t$X, centroids.t$Y, col = "red", pch = 19, cex = 0.1)

# 2/3 median nearest neighbor distance
mesh.median <- make_mesh(centroids.t, xy_cols = c("X", "Y"), cutoff = cutoff)  
plot(mesh.median)
points(centroids.t$X, centroids.t$Y, col = "red", pch = 19, cex = 0.1)

# round to nearest km from 2/3 median nearest neighbor distance
mesh.median.mod <- make_mesh(centroids.t, xy_cols = c("X", "Y"), cutoff = 4000)  
...
nlminb_loops
    ...
    Typical values: 1 (default), 2–3 for better convergence.

📌 Think of this as a way to refine the fit by looping through the optimizer multiple times.
...
🧮 newton_loops
    ...
    This is especially helpful when the Hessian is not positive definite, which is exactly the problem you’re encountering.

    Typical values: 1–5.

📌 Think of this as a way to clean up numerical issues and get a better estimate of uncertainty (standard errors, covariances, etc).
