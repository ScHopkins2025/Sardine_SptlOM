...
# Project data
dat2 <- dat %>%
  filter(Year > 2002 & survey == 'cps') %>% 
  distinct() %>%
  group_by(Year) %>%
  mutate(hauls = n_distinct(haul)) %>%
  ungroup() %>% 
  select(Year,cruise, haul, lat, hauls) %>% 
  distinct()
  View(dat2)

test2003 <- dat %>% filter(Year == 2003 & survey == 'cps')
...
land.barrier <- read_sf(here(dir, "Data", "Shapefiles", "gshhg-shp-2.3.7",
  ...
  st_transform(crs = st_crs(area2poly.dens2))

plot(land.barrier)                        
                        
mesh <- make_mesh(area2poly.dens2, xy_cols = c("X", "Y"), cutoff = 75, 
                  barrier_sf = land.barrier)  
