...
## and length bin
fit0 <- sdmTMB(Density ~ as.factor(age) + as.factor(len_bin),
               data = train, 
               mesh = barrier,
...
#### ML Criterion: 9351.420 (Best used for model comparison, not absolute assessment)
...
summary(lm(Density ~ est, data = pTrain0))$r.squared # 0.005012378
summary(lm(Density ~ est, data = pTest0))$r.squared # 1.401812e-05

# MAE
mean(abs(pTrain0$Density - pTrain0$est)) # 413.7007
mean(abs(pTest0$Density - pTest0$est)) # 396.4788
...
## Note that this is testing presence absence and then density predicted by age
## and length bin
fit1 <- sdmTMB(Density ~ as.factor(age) + as.factor(len_bin),
               data = train, 
               mesh = barrier,
               # Fit model by year and month
...
# Overall R2 for test and train 
summary(lm(Density ~ est, data = pTrain1))$r.squared # 0.1437345
summary(lm(Density ~ est, data = pTest1))$r.squared # 8.934803e-05

# MAE
mean(abs(pTrain1$Density - pTrain1$est)) # 280.5764
mean(abs(pTest1$Density - pTest1$est)) # 1.094452
...
fit2 <- sdmTMB(Density ~ as.factor(age) + as.factor(len_bin),
               data = train, 
               mesh = barrier,
               # Fit model by year and month
               ## Note if time is specified, spatiotemporal correlation is tested
...
# Overall R2 for test and train 
summary(lm(Density ~ est, data = pTrain2))$r.squared # 0.1437345
summary(lm(Density ~ est, data = pTest2))$r.squared # 8.934804e-05

# MAE
mean(abs(pTrain2$Density - pTrain2$est)) # 280.5764
mean(abs(pTest2$Density - pTest2$est)) # 1.094452
...
fit3 <- sdmTMB(Density ~ as.factor(age) + as.factor(len_bin), 
               data = train, 
               mesh = barrier,
...
# Predict (gives a whole new df, unlike GAM)
pTrain3 <- predict(fit3, newdata = train, type = "response")
pTest3 <- predict(fit3, newdata = test, type = "response")

# Overall R2 for test and train 
summary(lm(Density ~ est, data = pTrain3))$r.squared # 0.1406107
summary(lm(Density ~ est, data = pTest3))$r.squared # 5.224278e-06
...
# Now build the Seasonal models:

## For a "Non-spatial" model without covars

This is a test run to see if we can explain the distribution by discrete time and OM spatial area 

```{r, make.base.seasonal.nonsptl.model}
...
## Note that this is testing presence absence and then density predicted by age
## and length bin
fit4 <- sdmTMB(Density ~ as.factor(age) + as.factor(len_bin),
               data = train2, 
               mesh = barrier2,
               # Fit model by year and month
               time = "YearSeas.nm",
...
# Predict (gives a whole new df, unlike GAM)
pTrain4 <- predict(fit4, newdata = train2, type = "response")

pTest4 <- predict(fit4, newdata = test2, type = "response")

# Overall R2 for test and train 
summary(lm(Density ~ est, data = pTrain4))$r.squared # 0.005737915
summary(lm(Density ~ est, data = pTest4))$r.squared # 1.419252e-05

# MAE
mean(abs(pTrain4$Density - pTrain4$est)) # 457.6253
mean(abs(pTest4$Density - pTest4$est)) # 415.5279
...
## Note that this is testing presence absence and then density predicted by age
## and length bin
fit5 <- sdmTMB(Density ~ as.factor(age) + as.factor(len_bin),
               data = train2, 
               mesh = barrier2,
               # Fit model by year and month
               ## Note if time is specified, spatiotemporal correlation is tested
...
# Predict (gives a whole new df, unlike GAM)
pTrain5 <- predict(fit5, newdata = train2, type = "response")

pTest5 <- predict(fit5, newdata = test2, type = "response")

# Overall R2 for test and train 
summary(lm(Density ~ est, data = pTrain5))$r.squared # 0.1411138
summary(lm(Density ~ est, data = pTest5))$r.squared # 8.335531e-05
...
## Note that this is testing presence absence and then density predicted by age
## and length bin so we want the intercept
fit6 <- sdmTMB(Density ~ as.factor(age) + as.factor(len_bin),
               data = train2, 
               mesh = barrier2,
               # Fit model by year and month
               ## Note if time is specified, spatiotemporal correlation is tested
...
# Predict (gives a whole new df, unlike GAM)
pTrain6 <- predict(fit6, newdata = train2, type = "response")

pTest6 <- predict(fit6, newdata = test2, type = "response")

# Overall R2 for test and train 
summary(lm(Density ~ est, data = pTrain6))$r.squared # 0.140585
summary(lm(Density ~ est, data = pTest6))$r.squared # 2.217959e-06

# MAE
mean(abs(pTrain6$Density - pTrain6$est)) # 314.6031
mean(abs(pTest6$Density - pTest6$est)) # 0.9113865
...
```{r, make.base.seasonal.sptl.model}

set.seed(2)

fit7 <- sdmTMB(Density ~ as.factor(age) + as.factor(len_bin), 
               data = train2, 
               mesh = barrier2,
               # Fit model by year and quarter
               time = "YearSeas.nm",
...
#### Spatial SD: 510.60
## M2:
#### Dispersion parameter: 0.33
#### Matérn range: 111.90
#### Spatial SD: 52.69
#### ML criterion at convergence: 6888.455

# Predict (gives a whole new df, unlike GAM)
pTrain7 <- predict(fit7, newdata = train2, type = "response")
pTest7 <- predict(fit7, newdata = test2, type = "response")

# Overall R2 for test and train 
summary(lm(Density ~ est, data = pTrain7))$r.squared # 0.1012595
summary(lm(Density ~ est, data = pTest7))$r.squared # 3.592542e-06

# MAE
mean(abs(pTrain7$Density - pTrain7$est)) # 294.2342
mean(abs(pTest7$Density - pTest7$est)) # 24.7259
