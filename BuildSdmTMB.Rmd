...
area2poly.dens <- areaTranBuffsum.polygons %>%
  ...
  mutate(YearMon.nm = as.numeric(as.Date(paste0(YearMon, "-01"), 
                                         format = "%Y - %m - %d"))) %>%
  ungroup() %>%
...
tims <- data.frame("YearMon.nm" = unique(area2poly.dens2$YearMon.nm))
...
train <- subset(area2poly.dens2, YearMon.nm %in% trainTimStep) 
table(train$YearMon.nm) 

# Create test set
testTimStep <- tims[-index,]
test <- subset(area2poly.dens2, YearMon.nm %in% testTimStep) 
...
# Use previous study, cuttoff method above was likely overfitting
## Note both were run on a non-sptl model and returned the same results
...
fit0 <- sdmTMB(Density ~ Area_id, 
               data = train,
               mesh = barrier,
               time = "YearMon.nm",
               spatial = "off", 
               extra_time = test$YearMon.nm,
...
summary(lm(Density ~ est, data = pTrain0))$r.squared # 0.7934136
summary(lm(Density ~ est, data = pTest0))$r.squared # 0.008610168

# MAE
mean(abs(pTrain0$Density - pTrain0$est)) # 10.36276
mean(abs(pTest0$Density - pTest0$est)) # 11.8681
